...
export default function SignInPage() {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      // 1️⃣ Check if user exists & is an approved admin
      ...
      const currentUser = userRes.data.msg.find((m: any) => m.member === username)
      ...
      // 2️⃣ Verify passcode
      const passcodeRes = await axios.post(
        "https://espoint.onrender.com/espoint/confirm_member_passcode",
        {
          member: username,
          passcode: passcode
        },
      ...
      if (passcodeRes.data.error) {
        throw new Error(passcodeRes.data.error)
      }

      // 3️⃣ Store user info in localStorage & redirect
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-1">
          ...
          <CardDescription className="text-center">
            Enter your admin credentials to access the member management system
          </CardDescription>
        </CardHeader>
        <CardContent>
...
export default function SignInPage() {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      // 1️⃣ Check if user exists & is approved
      const checkUserUrl = `https://espoint.onrender.com/espoint/get_members_records/${username}/approved`
      const userRes = await axios.get(checkUserUrl)
      ...
      // 2️⃣ Store user info in localStorage & redirect
      localStorage.setItem(
        "user",
        JSON.stringify({
          username,
