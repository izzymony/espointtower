...
export default function SettingsPage() {
  ...
  const [formData, setFormData] = useState({
    name: "",
    labels: "regular" as Member["labels"],
  ...
  // Fetch members (uses new auto-detect API logic)
  ...
  const handleAddMember = async () => {
    ...
    try {
      ...
      await memberApi.createMember(username, username, formData.name, formData.labels, formData.passcode, formData.status);
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>List of members with who added them.</CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
            <p>Loading members...</p>
            ...
            <Table>
              ...
              <TableBody>
                {members.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={role === "admin" ? 5 : 4} className="text-center">No members found.</TableCell>
                  </TableRow>
                ) : (
                  members.map((m, idx) => (
                    <TableRow key={idx}>
                      ...
                      <TableCell><Badge>{m.labels}</Badge></TableCell>
...
export default function SettingsPage() {
  ...
  // Load members on mount and when filter changes
  useEffect(() => {
    if (!username) {
      alert("You must be logged in to access this page.");
      router.push("/dashboard");
  ...
  const handleAddMember = async () => {
    ...
    try {
      if (role !== "admin") throw new Error("Only admins can add members");
      await memberApi.createMember(username, username, formData.name, formData.labels, formData.passcode, formData.status);
      fetchMembers(filterStatus); // refresh table
      setIsAddDialogOpen(false);
      setFormData({ name: "", labels: "regular", status: "pending", passcode: "" });
    } catch (err: any) {
      alert(err.message);
  ...
  const handleDeleteMember = async (memberName: string) => {
    ...
    try {
      if (role !== "admin") throw new Error("Only admins can delete members");
      await memberApi.deleteMember(username, username, memberName);
      setMembers((prev) => prev.filter((m) => m.name !== memberName)); // instant UI update
    } catch (err: any) {
      alert(err.message);
    }
  };
  ...
  const handleEditMember = async () => {
    ...
    try {
      ...
      await memberApi.updateMemberLabel(username, username, editingMember.name, formData.labels);
      await memberApi.changeMemberStatus(editingMember.name, formData.status, username, username);
      await memberApi.changeMemberPasscode(editingMember.name, formData.status, formData.passcode, username, )

      setMembers((prev) =>
        prev.map((m) =>
          m.name === editingMember.name
            ? { ...m, labels: formData.labels, status: formData.status }
            : m
        )
      );
            ...
            <DialogContent>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2"><Label>Labels</Label>
                  <Select value={formData.labels} onValueChange={(value: Member["labels"]) => setFormData({ ...formData, labels: value })}>
                    <SelectTrigger><SelectValue/></SelectTrigger>
                    <SelectContent>{LABELS.map((label) => <SelectItem key={label.value} value={label.value}>{label.label}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
        ...
        <CardHeader>
          ...
          <div className="flex gap-2">
            ...
            <Select value={filterStatus} onValueChange={(value) => setFilterStatus(value)}>
              <SelectTrigger className="w-[150px]"><SelectValue placeholder="Select status"/></SelectTrigger>
              <SelectContent>
          ...
          <Table>
            ...
            <TableBody>
              {members.filter(m => filterStatus === "approved" || m.status === filterStatus).length === 0 ?
                <TableRow><TableCell colSpan={role === "admin" ? 5 : 4} className="text-center">No members found.</TableCell></TableRow> :
                members.filter(m => filterStatus === "approved" || m.status === filterStatus).map((m, idx) => (
                  <TableRow key={idx}>
                    <TableCell>{m.name}</TableCell>
                    <TableCell>{m.addedBy || "-"}</TableCell>
                    <TableCell><Badge>{m.labels}</Badge></TableCell>
                    ...
                    {role === "admin" && (
                      <TableCell className="flex gap-2">
                        <Button size="sm" variant="outline" onClick={() => openEditDialog(m)}><Edit className="h-4 w-4"/></Button>
                        <Button size="sm" variant="outline" onClick={() => handleDeleteMember(m.name)}><Trash2 className="h-4 w-4"/></Button>
                      </TableCell>
                    )}
                  </TableRow>
                ))
      ...
      {role === "admin" && (
        <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
          <DialogContent>
            ...
            <div className="space-y-4">
              <div className="space-y-2"><Label>Name</Label><Input value={formData.name} disabled/></div>
              <div className="space-y-2"><Label>Labels</Label>
                <Select value={formData.labels} onValueChange={(value: Member["labels"]) => setFormData({ ...formData, labels: value })}>
                  <SelectTrigger><SelectValue/></SelectTrigger>
                  <SelectContent>{LABELS.map((label) => <SelectItem key={label.value} value={label.value}>{label.label}</SelectItem>)}</SelectContent>
